...
class App {
    ...
    public function workerman_web_dispatch() {
        $uri = $_SERVER['REQUEST_URI'];
        if (($pos = strpos($uri, '?')) !== false) {
            $uri = substr($uri, 0, $pos);
        }
    ...
    public function workerman_run() {
        /* 初始化 */
        $this->init();
        /* URL调度 */
        $this->workerman_web_dispatch();
        /* 执行 */
        $class_name = $this->app_name . "\\action\\" . $this->module_name;
        $action = $this->action_name;
        ...
        if (!class_exists($class_name)) {
            $class_name = $this->app_name . "\\action\\_empty";
            if (!class_exists($class_name)) {
                return ['ret' => 404, 'data' => null, 'msg' => '控制器不存在'];
            }
        }
        ...
        if (!is_callable([$module, $action])) {
            if (is_callable([$module, '_empty'])) {
                ...
                return ['ret' => 404, 'data' => null, 'msg' => '方法不存在'];
            }
        }
