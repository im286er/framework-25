...
class RestCURL {
    ...
    function __construct($url = '') {
        if (!$this->is_enabled()) {
            Log::write('cURL Class - PHP was not built with cURL enabled. Rebuild PHP with --with-curl to use cURL.', Log::EMERG);
        }
    ...
    public function execute() {
        ...
        if (!isset($this->options[CURLOPT_DNS_CACHE_TIMEOUT])) {
            $this->options[CURLOPT_DNS_CACHE_TIMEOUT] = 300;
        }
...
class RestClient {
    ...
    private $rest_server = 'http://api.vvjob.com/';
    private $format = 'json';
    ...
    public static function getInstance($config = []) {
        static $obj = [];
        $key = to_guid_string($config);
        if (!isset($obj[$key])) {
            $obj[$key] = new self($config);
        }
        return $obj[$key];
    }
    ...
    function __construct($config = []) {
        ...
        if (!empty($_SERVER['HTTP_USER_AGENT'])) {
            ...
            $this->user_agent = 'vvjob api client';
        }
    }
    ...
    public function get($uri, $params = [], $format = 'json') {
        if ($params) {
    ...
    public function post($uri, $params = [], $format = 'json') {
        return $this->_call('post', $uri, $params, $format);
    }

    public function put($uri, $params = [], $format = 'json') {
        return $this->_call('put', $uri, $params, $format);
    }

    public function delete($uri, $params = [], $format = 'json') {
    ...
    private function _call($method, $uri, $params = [], $format = NULL) {
        ...
        // 设置 UA
        $this->_curl->option(CURLOPT_USERAGENT, $this->user_agent);
